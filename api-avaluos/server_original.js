...
app.get('/api/avaluos/:id/pdf', async (req, res) => {
    try {
        ...
        if (plantillas.length > 0) {
            ...
            const startColumns = () => {
                if (!isInsideColumns) {
                    htmlContenidoDinamico += `<div style="column-count: 2; column-gap: 4%; column-fill: auto; orphans: 2; widows: 2;">`;
            ...
            bloques.forEach(bloque => {
                ...
                // Handle Page Break for seccion_comparables_valoracion
                if (bloque.tipo === 'seccion_comparables_valoracion') {
                    stopColumns();
                    htmlContenidoDinamico += `<div style="page-break-before: always; clear: both;"></div>`;
                } else if (bloque.tipo === 'seccion_areas_normatividad') {
                    stopColumns(); // just to ensure the page break is outside columns
                    htmlContenidoDinamico += `<div style="page-break-before: always; clear: both;"></div>`;
                }
                ...
                if (bloque.tipo === 'seccion_comparables_valoracion' || bloque.tipo === 'imagenes_portada' || bloque.tipo === 'seccion_croquis' || bloque.tipo === 'titulo' || bloque.tipo === 'texto_libre' || bloque.tipo === 'tabla_info') {
                    ...
                    startColumns();
                    // Just output the block, the CSS columns will handle the masonry flow natively!
                    // We strip out the .corp-row wrapper so the left/right divs are direct children of the columns
                    let strippedHtml = bloqueHtml.replace(/<div class="corp-row">/g, '').replace(/<\/div>\s*$/g, '');
                    htmlContenidoDinamico += strippedHtml;
                }
            });
            stopColumns();
