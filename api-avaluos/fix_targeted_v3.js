...
// We will use CSS columns, but with a CRITICAL addition: `break-before: column;` on the right side block!
// This guarantees that if the left side is short, the right side stays on the right column.
// And if the left side is long, it wraps to the right column, and the right side gets pushed to the next column!
...
// We must also remove the global forced page breaks from seccion_usos_propuestos and others to prevent massive dead space!
content = content.replace(
...
content = content.replace(/<\/style>/, `
                .corp-block-item { width: 100%; margin-bottom: 20px; }
            </style>`);
...
console.log("Successfully targeted CSS columns with break-before: column applied.");
